                ...
                <div class="col-12">
                    ...
                    <article class="heading-component">
                        <div class="heading-component-inner">
                            <h5 class="heading-component-title">Tournaments</h5>
                    ...
                    <div id="tournaments-container">
                        ...
                        <div id="tournaments">
                            @forelse($tournaments as $tournament)
                                @include('frontend.partials.tournament-card', ['tournament' => $tournament])
                            @empty
                                <div class="d-flex justify-content-center">
                                    <h6>No tournaments available.</h6>
                                </div>
                            @endforelse
                        </div>
                    </div>
...
<script>
    $(document).on('change', '.filter-input', function () {
        let tournamentsDivSelector = $('#tournaments');
        let spinnerContainer = $('#spinner-container');

        tournamentsDivSelector.empty();
        spinnerContainer.removeClass('d-none')
        $.ajax({
            url: "{{ route('frontend.get-tournaments') }}",
            method: 'post',
            processing: true,
            serverSide: true,
            cache: true,
            ...
            {
                let tournamentsHtml = data.tournamentsHtml;
                tournamentsDivSelector.append(tournamentsHtml);
            },
