<body>
    ...
    <div wire:ignore class="card">
        <div class="card-header border-bottom">
            <h4 class="card-title mb-3">Receipt</h4>
            <div class="d-flex justify-content-between align-items-center row pb-2 gap-3 gap-md-0">
                <div class="col-md-4"></div>
            </div>
        </div>
        <div class="card-datatable table-responsive">
            <table class="datatables-receipt table border-top">
                <thead>
                 <tr>
                    <th></th>
    ...
    <script>
        document.addEventListener('livewire:navigated', function () {
            ...
            if (dt_receipt_table.length) {
                var dt_receipt = dt_receipt_table.DataTable({
                    data: receipt,
                    columns: [
                        { data: '' },
                    ...
                    columnDefs: [
                        ...
                        
                        {
                            ...
                            targets: 1,
                            render: function (data, type, full, meta) {
                                var $user = full['user']['username'];

                                return '<span class="fw-semibold">' + $user + '</span>';
                            }
                        },
                        {
                            targets: 2,
                            render: function (data, type, full, meta) {
                                var $paid_by = full['paid_by'];

                                return '<span class="fw-semibold">' + $paid_by + '</span>';
                            }
                        },
...

                        {
                            ...
                            orderable: false,
                            render: function (data, type, full, meta) {
                                let editLink = "{{ route('receipt.edit', '%id%') }}".replace('%id%', full['id']);
                                let viewLink = "{{ route('receipt.view', ['id' => '%id%', 'status' => '%status%']) }}".replace('%id%', full['id']).replace('%status%', 1);
                    ...
                    buttons: [
                        ...
                        @can('user-create')
                        {
                            ...
                            attr: {
                                'data-href': "{{ route('receipt.create') }}",
                            }
                        }
                        @endcan
                    ],
