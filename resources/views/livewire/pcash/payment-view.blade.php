<body>
    ...
    <script>
        document.addEventListener('livewire:navigated', function () {
            var dt_payment_table = $('.datatables-payment')

            var payment = @json($payment ?? []);
            if (dt_payment_table.length) {
                var dt_payment = dt_payment_table.DataTable({
                    data: payment,
                    columns: [
                        { data: '' },
                    ...
                    columnDefs: [
                        ...
                        
                        {
                            ...
                            targets: 1,
                            render: function (data, type, full, meta) {
                                var $category_id = full['category']['category_name'];

                                return '<span class="fw-semibold">' + $category_id + '</span>';
                            }
                        },
                        {
                            targets: 2,
                            render: function (data, type, full, meta) {
                                var subCategory = full.sub_category;  
                                var $subCategory_id = subCategory && subCategory.sub_category_name;
                                    return '<span class="fw-semibold">' + $subCategory_id + '</span>';
...

                        {
                            ...
                            orderable: false,
                            render: function (data, type, full, meta) {
                                let editLink = "{{ route('payment.edit', '%id%') }}".replace('%id%', full['id']);
                                let viewLink = "{{ route('payment.view', ['id' => '%id%', 'status' => '%status%']) }}".replace('%id%', full['id']).replace('%status%', 1);
